  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1 text-sm cursor-pointer text-purple-600 bg-gradient-to-br from-purple-50 to-purple-100 ring-purple-300 hover:ring transition-all px-3 py-2 rounded-lg"
      >
        <Palette size={16} />
      ...
      {isOpen && (
        <div className="grid grid-cols-4 w-60 gap-2 absolute top-full left-0 right-0 p-3 mt-2 z-10 bg-white rounded-md border border-gray-200 shadow-sm">
          {colors.map((color: { name: string; value: string }) => (
            ...
            >
              <div
                className="h-11 w-11 rounded-full border-2 border-transparent group-hover:border-black/25 transition-colors"
              ...
              <p className="text-xs text-center mt-1 text-gray-600s">
                {color.name}
              </p>
            </div>
          ))}
